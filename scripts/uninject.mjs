/*
 * Globalcord — Local un-injector for Discord Desktop
...
function findAllDiscordResources() {
    ...
    // Filtrer uniquement les paths avec une injection Globalcord présente
...
function uninject(resourcesDir) {
    ...
    // Vérifier que le dossier app/ a bien été créé par Globalcord
    if (existsSync(appDirPath)) {
        try {
            const pkg = JSON.parse(readFileSync(join(appDirPath, "package.json"), "utf-8"));
            if (pkg.name !== "globalcord") {
                console.warn(`\x1b[33m[Globalcord] Le dossier app/ existe mais n'a pas été créé par Globalcord (name: "${pkg.name}").\x1b[0m`);
                console.warn("\x1b[33m            Abandon pour éviter de casser un autre mod.\x1b[0m");
                return false;
            }
        } catch { }

        console.log("[Globalcord] Suppression du dossier app/ injecté...");
        rmSync(appDirPath, { recursive: true, force: true });
    } else {
        console.log("\x1b[33m[Globalcord] Aucun dossier app/ injecté trouvé.\x1b[0m");
    }

    // Restaurer le backup
    if (existsSync(backupPath) && !existsSync(appAsarPath)) {
        console.log("[Globalcord] Restauration _app.asar → app.asar...");
        renameSync(backupPath, appAsarPath);
    } else if (existsSync(backupPath) && existsSync(appAsarPath)) {
        console.log("[Globalcord] app.asar déjà présent, nettoyage du backup...");
        rmSync(backupPath, { force: true });
    }

    console.log(`\x1b[32m[Globalcord] Désinjection réussie depuis : ${resourcesDir}\x1b[0m`);
...
if (allResources.length === 0) {
    console.error("\x1b[31m[Globalcord] Aucune installation Discord avec Globalcord injecté trouvée.\x1b[0m");
    console.error("\x1b[33m           Assurez-vous que Globalcord a bien été injecté via 'pnpm inject'.\x1b[0m");
    process.exit(1);
}

let uninjectCount = 0;
for (const res of allResources) {
    console.log(`\n[Globalcord] Trouvé : ${res}`);
    if (uninject(res)) uninjectCount++;
}

if (uninjectCount === 0) {
    console.error("\x1b[31m[Globalcord] Aucune désinjection réussie.\x1b[0m");
    process.exit(1);
}

console.log(`\n\x1b[32m[Globalcord] ${uninjectCount}/${allResources.length} désinjection(s) réussie(s).\x1b[0m`);
