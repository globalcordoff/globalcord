/*
 * Globalcord — Installer via EquilotlCli
 * Télécharge EquilotlCli.exe depuis les releases Equicord et le lance
 * avec les variables d'environnement pointant vers les fichiers Globalcord.
 *
 * L'exe affiche une interface graphique permettant de choisir le Discord cible.
 *
 * Usage:
 *   pnpm inject    → installe Globalcord dans le Discord choisi
 *   pnpm uninject  → désinstalle Globalcord du Discord choisi
...
// EquilotlCli est l'installeur graphique d'Equicord — on le réutilise pour Globalcord
...
async function ensureBinary() {
    ...
    if (existsSync(outputFile)) {
        console.log("[Globalcord] Installeur déjà présent, utilisation locale.");
        return outputFile;
    }

    console.log("[Globalcord] Téléchargement de l'installeur (" + filename + ")...");

    const res = await fetch(BASE_URL + filename, {
        headers: {
            "User-Agent": "Globalcord (https://github.com/globalcordoff/globalcord)"
        }
    });
    ...
    if (process.platform === "darwin") {
        console.log("[Globalcord] Sauvegarde du zip...");
        const zip = new Uint8Array(await res.arrayBuffer());
        writeFileSync(downloadName, zip);

        console.log("[Globalcord] Extraction du bundle...");
        execSync(`ditto -x -k '${downloadName}' '${FILE_DIR}'`);

        console.log("[Globalcord] Suppression de la quarantaine macOS...");
        const logAndRun = cmd => {
    ...
    console.log("[Globalcord] Installeur téléchargé avec succès !");
    return outputFile;
}
...
function checkBuild() {
    ...
    if (!existsSync(patcherPath)) {
        console.error("\x1b[31m[Globalcord] dist/desktop/patcher.js introuvable !\x1b[0m");
...
function cleanOldGlobalcord() {
    console.log("[Globalcord] Recherche et nettoyage automatique des anciennes installations...");
    ...
    for (const resourcesDir of candidates) {
        ...
        try {
            ...
            // 1. Supprimer le dossier app/ s'il a été créé par l'ancien Globalcord
            if (existsSync(appDirPath)) {
                ...
                if (shouldDelete) {
                    console.log(`[Globalcord] Suppression de l'ancien dossier app/ dans : ${resourcesDir}`);
            ...
            if (existsSync(backupPath)) {
                ...
                if (isAsarDir) {
                    console.log(`[Globalcord] Suppression du dossier app.asar temporaire dans : ${resourcesDir}`);
                    rmSync(appAsarPath, { recursive: true, force: true });
                }

                if (!existsSync(appAsarPath) || isAsarDir) {
                    console.log(`[Globalcord] Restauration _app.asar -> app.asar dans : ${resourcesDir}`);
                    ...
                    console.log(`[Globalcord] Nettoyage du backup _app.asar obsolète dans : ${resourcesDir}`);
                    rmSync(backupPath, { force: true });
                    cleanedAny = true;
                }
            }
        } catch (e) {
            console.error(`[Globalcord] Erreur lors du nettoyage de ${resourcesDir} :`, e.message);
        }
    }

    if (cleanedAny) {
        console.log("[Globalcord] Nettoyage des anciennes installations terminé avec succès !");
    } else {
        console.log("[Globalcord] Aucune ancienne installation à nettoyer.");
    }
}

// ── Main ─────────────────────────────────────────────────────────────────────
// On nettoie d'abord les anciennes traces de Globalcord pour éviter tout conflit ou blocage
cleanOldGlobalcord();
...
if (isInstall) {
    console.log("[Globalcord] Nettoyage des installations précédentes (Vencord/Equicord/Globalcord)...");
    try {
        ...
        console.log("[Globalcord] Nettoyage terminé.");
    } catch {
        console.log("[Globalcord] Aucun mod précédent à nettoyer ou échec du nettoyage.");
    }
}

console.log("[Globalcord] Lancement de l'injection...");
...
try {
    execFileSync(installerBin, args, {
        ...
        env: {
            ...
            EQUICORD_DIRECTORY: join(BASE_DIR, "dist", "desktop"),
            EQUICORD_DEV_INSTALL: "1",
            GLOBALCORD_DIRECTORY: join(BASE_DIR, "dist", "desktop")
        }
    });
} catch {
    console.error("[Globalcord] Erreur lors de l'injection.");
    process.exit(1);
}
