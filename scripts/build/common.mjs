...
export const gitHash = process.env.GLOBALCORD_HASH || execSync("git rev-parse HEAD", { encoding: "utf-8" }).trim();
...
export const globPlugins = kind => ({
    ...
    setup: build => {
        ...
        build.onLoad({ filter, namespace: "import-plugins" }, async () => {
            const pluginDirs = ["plugins/_api", "plugins/_core", "plugins", "userplugins", "globalcordplugins", "globalcordplugins/_api"];
            let code = "";
            let pluginsCode = "\n";
            let metaCode = "\n";
            let excludedCode = "\n";
...
export const gitRemotePlugin = {
    ...
    setup: build => {
        ...
        build.onLoad({ filter, namespace: "git-remote" }, async () => {
            let remote = process.env.GLOBALCORD_REMOTE;
            if (!remote) {
...
export const commonOpts = {
    ...
    alias: {
        ...
        "@globalcordplugins": "./src/globalcordplugins",
    }
};
