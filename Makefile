...
RC_OBJ = $(BUILD_DIR)/globalcord_rc.o
...
all: $(BUILD_DIR)/globalcord $(BUILD_DIR)/updater.exe
...
$(BUILD_DIR)/globalcord_rc.o: globalcord.rc assets/globalcord.ico
	mkdir -p $(BUILD_DIR)
	windres globalcord.rc -o $@

$(BUILD_DIR)/%.o: %.c
	mkdir -p $(dir $@)
	$(CC) -c -o $@ $(FLAGS) $< -Wall -Wno-unused-function -Wno-misleading-indentation $(OPTS)

$(BUILD_DIR)/globalcord: $(OBJS) $(BUILD_DIR)/resources.o $(RC_OBJ) $(BUILD_DIR)/themes/ $(BUILD_DIR)/sounds/
...
uninstall:
	rm -f $(PREFIX)/share/applications/globalcord.desktop
	rm -f $(PREFIX)/share/pixmaps/globalcord.png
	rm -f $(PREFIX)/share/pixmaps/globalcord.svg
	rm -f $(PREFIX)/bin/globalcord

install: uninstall
	cp globalcord.desktop $(PREFIX)/share/applications/globalcord.desktop
	cp assets/icon.svg $(PREFIX)/share/pixmaps/globalcord.svg
	cp $(BUILD_DIR)/globalcord $(PREFIX)/bin/globalcord

run: all
	cp assets/globalcord.gschema.xml $(BUILD_DIR)/assets/globalcord.gschema.xml
