# ==============================================================================
#  Globalcord — Installeur utilisateur (PowerShell autonome)
#  
#  Ce script fait TOUT automatiquement :
#  1. Télécharge EquilotlCli.exe (outil d'injection graphique)
#  2. Télécharge les fichiers Globalcord compilés depuis GitHub
#  3. Lance l'interface graphique pour choisir votre Discord cible
#  4. Injecte Globalcord dans Discord
#
...
# ── Configuration ─────────────────────────────────────────────────────────────
$GlobalcordRepo   = "globalcordoff/globalcord"
$EquilotlUrl     = "https://github.com/Equicord/Equilotl/releases/latest/download/EquilotlCli.exe"
$InstallDir      = Join-Path $env:LOCALAPPDATA "Globalcord"
...
function Write-Banner {
    ...
    Write-Host "  ║          GLOBALCORD  INSTALLER            ║" -ForegroundColor Cyan
...
if (Test-Path $EquilotlExe) {
    ...
    try {
        $head = Invoke-WebRequest -Uri $EquilotlUrl -Method Head -UseBasicParsing `
            -Headers @{ "User-Agent" = "Globalcord-Installer/2.0" }
...
# ── [2/3] Télécharger les fichiers Globalcord ──────────────────────────────────
Write-Step 2 3 "Téléchargement des fichiers Globalcord depuis GitHub..."

try {
    $apiUrl   = "https://api.github.com/repos/$GlobalcordRepo/releases/latest"
    $release  = Invoke-RestMethod -Uri $apiUrl -UseBasicParsing `
        -Headers @{ "User-Agent" = "Globalcord-Installer/2.0"; "Accept" = "application/vnd.github.v3+json" }

    $version  = $release.tag_name
    $distAsset = $release.assets | Where-Object { $_.name -eq "globalcord-dist.zip" } | Select-Object -First 1

    if (-not $distAsset) {
        Write-Fail "Fichier 'globalcord-dist.zip' introuvable dans la release $version.`n           Contactez le support Globalcord."
    }

    Write-Host "          Version : $version" -ForegroundColor DarkGray
    Write-Host "          Téléchargement en cours..." -ForegroundColor DarkGray

    $zipPath = Join-Path $InstallDir "globalcord-dist.zip"
    ...
    Write-OK "Globalcord $version prêt à être injecté !"
} catch {
    Write-Fail "Échec du téléchargement Globalcord.`n           Détail : $_"
}
...
Write-Host "          │  Sélectionnez le Discord où injecter Globalcord. │" -ForegroundColor DarkCyan
Write-Host "          └─────────────────────────────────────────────────┘" -ForegroundColor DarkCyan
Write-Host ""
...
Write-Host "  ║  Globalcord installé avec succès !                    ║" -ForegroundColor Green
Write-Host "  ║                                                      ║" -ForegroundColor Green
Write-Host "  ║  → Redémarrez Discord pour appliquer Globalcord.      ║" -ForegroundColor Green
Write-Host "  ╚══════════════════════════════════════════════════════╝" -ForegroundColor Green
Write-Host ""
Write-Host "  Pour désinstaller : exécutez globalcord-uninstall.bat" -ForegroundColor DarkGray
Write-Host ""
