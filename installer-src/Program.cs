...
namespace GlobalcordInstaller
{
    ...
    public class LauncherForm : Form
    {
        ...
        private GlobalcordBackend _backend;

        public LauncherForm()
        {
            this.Text = "Globalcord Installer";
            ...
            var iconStream = Assembly.GetExecutingAssembly().GetManifestResourceStream("GlobalcordInstaller.icon.ico");
        ...
        {
            var userDataFolder = Path.Combine(Path.GetTempPath(), "GlobalcordInstaller_WebView2");
            ...
            )
            {
                ...
                MessageBox.Show(
                    "Microsoft Edge WebView2 Runtime is required to run the Globalcord Installer but was not found on your system.\n\n" +
                    "Please download and install it from:\nhttps://aka.ms/webview2\n\n" +
                    "After installing, restart the Globalcord Installer.",
            ...
            _backend = new GlobalcordBackend(this, _webView);
            ...
            using (var stream = Assembly.GetExecutingAssembly().GetManifestResourceStream("GlobalcordInstaller.index.html"))
            using (var reader = new StreamReader(stream))
            {
                ...
                try {
                    using (var iconStream = Assembly.GetExecutingAssembly().GetManifestResourceStream("GlobalcordInstaller.icon.ico"))
                    {
    ...
    [ComVisible(true)]
    public class GlobalcordBackend
    {
        ...
        const string GITHUB_REPO = "globalcordoff/globalcord";
        const string DIST_ZIP = "globalcord-dist.zip";

        public GlobalcordBackend(LauncherForm form, WebView2 webView)
        {
            _form = form;
            _webView = webView;
            ...
            _distDir = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Globalcord", "dist");
        }
        ...
        public async Task<bool> IsInjected(string path)
        {
            return await Task.Run(() => {
                ...
                return Directory.Exists(appDir) && File.Exists(pkgPath) && File.ReadAllText(pkgPath).Contains("\"globalcord\"");
            });
        }

        /// <summary>
        /// Returns true if a third-party mod (Vencord, Equicord, OpenAsar) is detected
        /// but Globalcord is NOT yet injected.
        /// </summary>
        public async Task<bool> HasThirdPartyMod(string path)
        {
            return await Task.Run(() => {
                ...
                // Already Globalcord → not a third-party mod
                if (content.Contains("\"globalcord\"")) return false;
        ...
        private async Task EnsureDistAsync()
        {
            ...
            _http.DefaultRequestHeaders.Add("User-Agent", "Globalcord-Installer/2.0");
                ...
                using (var fs = new FileStream(tmpZip, FileMode.Create, FileAccess.Write, FileShare.None, 81920, true))
                {
                    ...
                    while ((read = await contentStream.ReadAsync(buffer, 0, buffer.Length)) > 0)
                    {
                        ...
                        if (percent - lastReportedPercent >= 0.5 || percent >= 100.0)
                        {
                            ...
                            SetProgress(overallPercent, "Downloading Globalcord...", readMB, totalMB);
                        }
                    }
                }
            }
        ...
        private void DoInject(string resPath)
        {
            ...
            {
                // Déjà Globalcord → on réinjecte proprement sans bail-out partiel
            ...
            SetProgress(92, "Configuring Globalcord loader...");
            ...
            // Créer le backup Globalcord si app.asar existe et qu'on n'a pas encore de backup
        ...
        private void CleanModulePatches(string resPath)
        {
            ...
            catch (Exception ex)
            {
                ...
                Console.WriteLine($"[Globalcord] CleanModulePatches warning: {ex.Message}");
            }
        }
        ...
        private void DoUninject(string resPath)
        {
            ...
            if (Directory.Exists(appDir))
            {
                ...
                if (File.Exists(pkg) && File.ReadAllText(pkg).Contains("\"globalcord\""))
                {
            ...
            // 3. Clean up Globalcord-specific assets (folders we added)
        ...
        private void WriteLoader(string appDir)
        {
            ...
            // _distDir = %LOCALAPPDATA%\Globalcord\dist\
            ...
            File.WriteAllText(Path.Combine(appDir, "package.json"), "{\"name\":\"globalcord\",\"main\":\"index.js\"}");
            File.WriteAllText(Path.Combine(appDir, "index.js"),
                ...
                $"if (!fs.existsSync(patcherPath)) throw new Error('[Globalcord] patcher.js not found. Expected at: ' + primary);\n" +
                $"require(patcherPath);\n"
            );
        }
