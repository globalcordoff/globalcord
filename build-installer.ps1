# build-installer.ps1 - Compile Globalcord-Installer.exe
...
$OutExe = Join-Path $OutDir "Globalcord-Installer.exe"

Write-Host ""
Write-Host "  [Globalcord] Compiling installer..." -ForegroundColor Cyan
...
if ($hasDotnet) {
    ...
    & dotnet publish "$SrcDir\GlobalcordInstaller.csproj" `
        -c Release `
        -o $OutDir `
        --nologo `
        -v quiet `
    ...
    $built = Get-ChildItem $OutDir -Recurse -Filter "Globalcord-Installer.exe" -ErrorAction SilentlyContinue | Select-Object -First 1
...
# Method 2: csc.exe (.NET Framework - always present on Windows)
    ...
    $ico    = Join-Path $Root "globalcord.ico"
...
if (Test-Path $OutExe) {
    ...
    Write-Host ""
    Write-Host "  OK  Globalcord-Installer.exe compiled ($size KB)" -ForegroundColor Green
    Write-Host "    -> $OutExe" -ForegroundColor DarkGray
    Write-Host ""
} else {
    Write-Host "  [ERROR] Globalcord-Installer.exe not found after compilation." -ForegroundColor Red
    exit 1
}
