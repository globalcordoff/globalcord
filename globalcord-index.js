...
// ── CRITIQUE : userData = dossier Globalcord pour les settings/plugins
...
const globalcordData = path.join(app.getPath("appData"), "Globalcord");
app.setPath("userData", globalcordData);
...
// pour le token Discord stable. Quand Globalcord charge ce cache, il lit des permissions
...
// AppUserModelId unique — Windows reconnaît Globalcord comme app séparée de Discord
...
app.once("ready", () => {
    try {
        ...
        //     Seuls les modules vraiment inutiles pour Globalcord sont bloqués.
        ...
        app.once("browser-window-created", () => {
            try {
                ...
                ses.webRequest.onBeforeRequest(
                    ...
                    (details, callback) => {
                        ...
                        if (isBlocked) {
                            ...
                            console.log("[Globalcord] Module bloqué (inutile pour Globalcord):", url.split("/").slice(-2).join("/"));
                ...
                console.log("[Globalcord] Filtre modules 403 activé ✓");
            } catch (e) {
                console.warn("[Globalcord] Impossible d'activer le filtre modules:", e.message);
            }
        });
    } catch (e) {
        console.warn("[Globalcord] FIX modules 403 failed:", e.message);
    }
});
...
try {
    const lsPath = path.join(globalcordData, "Local Storage", "leveldb");
    if (fs.existsSync(lsPath)) {
        ...
        if (corrupted) {
            console.warn("[Globalcord] LevelDB localStorage corrompu détecté — réparation...");
            try { fs.rmSync(lsPath, { recursive: true, force: true }); } catch { }
            console.warn("[Globalcord] LevelDB supprimé — les données localStorage seront récréées");
        }
    }
} catch (e) { console.warn("[Globalcord] LevelDB check failed:", e.message); }

// Modules bundlés dans globalcord-dist/modules/
...
try {
    ...
    if (entries.length > 0) {
        ...
        console.log("[Globalcord] Modules natifs Discord détectés:", discordNativeModulesPath);
    }
} catch (e) {
    console.warn("[Globalcord] Impossible de détecter les modules natifs Discord:", e.message);
}
...
// Priorité aux modules bundlés (portables, dans globalcord-dist/modules/)
...
if (discordNativeModulesPath) {
    ...
    } catch (e) { console.warn("[Globalcord] Erreur lors du scan des modules natifs:", e.message); }
}
try {
...
console.log("[Globalcord] mainAppDirname:", global.mainAppDirname);
...
try {
    ...
    if (fs.existsSync(nativeModulesDir) && !buildInfo.localModulesRoot) {
        ...
        console.log("[Globalcord] build_info.json patché → localModulesRoot:", nativeModulesDir);
    }
} catch (e) {
    console.warn("[Globalcord] Impossible de patcher build_info.json:", e.message);
}
