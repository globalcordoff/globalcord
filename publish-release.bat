@echo off
:: ─── Globalcord — Publier une nouvelle release sur GitHub ─────────────────────
...
if "%NOTES%"=="" set NOTES=Globalcord %VERSION%

:: Chemins de sortie
set DIST_DIR=dist\desktop
set OUT_DIR=release\installer
set DIST_ZIP=%OUT_DIR%\globalcord-dist.zip
set INSTALLER_EXE=%OUT_DIR%\Globalcord-Installer.exe
set VERSION_JSON=%OUT_DIR%\version.json
set DESKTOP_ASAR=dist\desktop.asar

echo.
echo  ╔═══════════════════════════════════════════════════╗
echo  ║    GLOBALCORD — Publication release v%VERSION%
echo  ╚═══════════════════════════════════════════════════╝
echo.
...
:: ── 5. Compiler Globalcord-Installer.exe ──────────────────────────────────────
echo.
...
if not exist "%INSTALLER_EXE%" (
    echo  [ERREUR] Globalcord-Installer.exe introuvable apres compilation.
    pause
    exit /b 1
)

for %%F in ("%INSTALLER_EXE%") do echo  [5/8] Globalcord-Installer.exe cree (%%~zF octets)

:: ── 6. Créer globalcord-dist.zip ──────────────────────────────────────────────
echo.
echo  [6/8] Creation de globalcord-dist.zip...
...
powershell -NoProfile -Command "Add-Type -Assembly System.IO.Compression.FileSystem; $src = (Resolve-Path '%DIST_DIR%').Path; $dst = (Join-Path (Resolve-Path 'release\installer').Path 'globalcord-dist.zip'); [System.IO.Compression.ZipFile]::CreateFromDirectory($src, $dst, [System.IO.Compression.CompressionLevel]::Optimal, $false)"

if not exist "%DIST_ZIP%" (
    echo  [ERREUR] Impossible de creer globalcord-dist.zip
    pause
    exit /b 1
)

for %%F in ("%DIST_ZIP%") do echo  [6/8] globalcord-dist.zip cree (%%~zF octets)
...
(
    ...
    echo   "installerUrl": "https://github.com/globalcordoff/globalcord/releases/latest/download/Globalcord-Installer.exe",
    echo   "distUrl": "https://github.com/globalcordoff/globalcord/releases/latest/download/globalcord-dist.zip",
    echo   "downloadUrl": "https://github.com/globalcordoff/globalcord/releases/latest/download/desktop.asar",
...
gh release create "v%VERSION%" ^
    "%INSTALLER_EXE%#Globalcord-Installer.exe" ^
    "%DIST_ZIP%#globalcord-dist.zip" ^
    "%DESKTOP_ASAR%#desktop.asar" ^
    "%VERSION_JSON%#version.json" ^
    --repo globalcordoff/globalcord ^
    --title "Globalcord v%VERSION%" ^
    --notes "!NOTES!" ^
    --latest
...
echo.
echo  ╔═══════════════════════════════════════════════════════════════╗
echo  ║  Globalcord v%VERSION% publie avec succes !
echo  ║
echo  ║  Fichiers publies sur GitHub :
echo  ║    Globalcord-Installer.exe    — installeur .exe avec GUI
echo  ║    globalcord-dist.zip         — JS obfusques (pour l'injec.)
echo  ║    version.json               — metadonnees de version
echo  ║
echo  ║  Les utilisateurs telechargeront Globalcord-Installer.exe
