...
// ─── Configuration de Build Globalcord ─────────────────────────────────────────

function killGlobalcord() {
    const releaseDir = join(__dirname, "release", "globalcord-dist");
...
function buildGlobalcordFromDiscord(discordApp) {
    ...
    console.log("[globalcord] Préparation de _app.asar...");
...
app.setPath("userData", path.join(app.getPath("appData"), "Globalcord"));
...
`);
    ...
    writeFileSync(join(outApp, "package.json"), JSON.stringify({ name: "globalcord", main: "index.js", version: "1.18.0" }, null, 2));
...
`);
    ...
    const globalcordPreload = join(__dirname, "globalcord-preload.js");
    if (existsSync(globalcordPreload)) {
        cpSync(globalcordPreload, join(outDist, "preload.js"));
    }
    ...
    const iconSrc = join(__dirname, "globalcord.ico");
    if (existsSync(iconSrc)) {
        ...
        try {
            ...
            if (existsSync(rcedit)) {
                execSync(`"${rcedit}" "${discordExe}" --set-icon "${iconSrc}" --set-version-string "ProductName" "Globalcord" --set-version-string "FileDescription" "Globalcord"`, { stdio: "ignore" });
            }
        } catch (e) { }
    }

    console.log(`[globalcord] Build terminé -> ${outDir}`);
}
...
buildGlobalcordFromDiscord(discord);

module.exports = {
    appId: "com.globalcord.app",
    productName: "Globalcord",
    copyright: "Copyright 2026 Globalcord",
    extraMetadata: { main: "dist/js/main.js" },
    asar: false,
    files: ["dist/js/**/*", "static/**/*", "package.json", "!**/*.map", "!**/*.ts"],
