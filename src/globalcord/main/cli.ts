...
const options = {
    ...
    repair: {
        type: "boolean",
        short: "r",
        description: "Re-download Globalcord and restart"
    }
...
const extraOptions = {
    ...
    "ozone-platform": {
        hidden: process.platform !== "linux",
        type: "string",
        description: "Whether to run Globalcord in Wayland or X11 (XWayland)",
        options: ["x11", "wayland"]
    }
} satisfies Record<string, Option>;
...
export async function checkCommandLineForRepair() {
    ...
    if (State.store.GlobalcordDir) {
        console.error("Cannot repair: using custom Globalcord directory. Remove it in settings first.");
        process.exit(1);
    }
...
export function checkCommandLineForHelpOrVersion() {
    ...
    if (version) {
        console.log(`Globalcord v${app.getVersion()}`);
        app.exit(0);
    }

    if (help) {
        const base = stripIndent`
            Globalcord v${app.getVersion()}
...
function checkCommandLineForToggleCommands() {
    ...
    console.error("Globalcord is not running. Toggle commands require a running instance.");
    app.exit(1);
}
...
function checkForSecondInstance() {
    ...
    if (!app.requestSingleInstanceLock({ IS_DEV })) {
        if (IS_DEV) {
            console.log("Globalcord is already running. Quitting previous instance...");
            return;
        } else {
            console.log("Globalcord is already running. Quitting...");
            app.exit(0);
        }
    }
