...
handleSync(IpcEvents.GET_GIT_HASH, () => Globalcord_GIT_HASH);
...
// Handler pour VencordNative.globalcord.relaunch() — utilisé par les boutons Restart dans les settings plugins
...
handleSync(IpcEvents.IS_USING_CUSTOM_VENCORD_DIR, () => !!State.store.GlobalcordDir);
handle(IpcEvents.SHOW_CUSTOM_VENCORD_DIR, async () => {
    const { GlobalcordDir } = State.store;
    if (!GlobalcordDir) return;

    const stats = await stat(GlobalcordDir);
    if (!stats.isDirectory()) return;

    shell.openPath(GlobalcordDir);
});
...
handle(IpcEvents.SELECT_VENCORD_DIR, async (_e, value?: null) => {
    if (value === null) {
        delete State.store.GlobalcordDir;
        return "ok";
    }
