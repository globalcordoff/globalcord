...
import { openModal } from "@Globalcord/types/utils";
import { onceReady } from "@Globalcord/types/webpack";
import { VoiceActions } from "@Globalcord/types/webpack/common";
...
import type SettingsPlugin from "@Globalcord/types/plugins/_core/settings";
import { VesktopSettingsIcon } from "shared/icons";

VesktopLogger.log("read if cute :3");
VesktopLogger.log(`Globalcord v${VesktopNative.app.getVersion()}`);

const { customEntries, customSections } = Vencord.Plugins.plugins.Settings as any as typeof SettingsPlugin;

customEntries.push({
    key: "Globalcord_Globalcord_settings",
    title: "Globalcord Settings",
    Component: SettingsUi,
    Icon: VesktopSettingsIcon
});

customSections.push(() => ({
    section: "GlobalcordSettings",
    label: "Globalcord Settings",
    element: SettingsUi,
    className: "vc-Globalcord-settings"
}));
