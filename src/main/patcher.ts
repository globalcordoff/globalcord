...
if (!IS_VANILLA) {
            ...
            if (options?.webPreferences?.preload && options.title) {
                ...
                if (process.platform === "win32" && winMaterial && winMaterial !== "none") {
                    try {
                        ...
                        if (!applied) {
                            console.warn("[Globalcord] No background material API available on this system");
                        }
                    } catch (e) {
                        console.error("[Globalcord] setBackgroundMaterial failed:", e);
                    }
                }
    ...
    process.env.DATA_DIR = join(app.getPath("userData"), "..", "Globalcord");
        ...
        (electron.ipcMain as any).handle = function(channel: string, listener: any) {
            ...
            try {
                ...
                if (e?.message?.includes?.("Attempted to register a second handler")) {
                    console.warn(`[Globalcord] Ignored duplicate IPC handler for '${channel}'`);
                    return;
                }
                throw e;
            }
    ...
    console.log("[Globalcord] Running in vanilla mode. Not loading Globalcord");
}

console.log("[Globalcord] Loading original Discord app.asar");
