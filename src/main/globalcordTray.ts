...
/**
 * Initializes the Globalcord system tray icon.
 ...
 */
export function initGlobalcordTray(injectorPath: string) {
    ...
    app.whenReady().then(() => {
        try {
            ...
            tray.setToolTip("Globalcord");

            const trayMenu = Menu.buildFromTemplate([
                {
                    label: "Open Globalcord",
                    click() {
                ...
                { type: "separator" },
                {
                    label: "Uninstall Globalcord",
                    click() {
                        const uninstallerCandidates = [
                            join(process.env.LOCALAPPDATA || "", "Programs", "globalcord", "Uninstall Globalcord.exe"),
                            join(process.env.LOCALAPPDATA || "", "Programs", "Globalcord", "Uninstall Globalcord.exe"),
                            join(process.env.PROGRAMFILES || "", "Globalcord", "Uninstall Globalcord.exe"),
                        ];
            ...
            console.log("[Globalcord] Tray created ✓", iconPath ?? "(icon from exe)");
        } catch (e) {
            console.error("[Globalcord] Failed to create tray:", e);
        }
    });
}
