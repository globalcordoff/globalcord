...
export const DATA_DIR = process.env.GLOBALCORD_USER_DATA_DIR ?? (
    process.env.DISCORD_USER_DATA_DIR
        ? join(process.env.DISCORD_USER_DATA_DIR, "..", "GlobalcordData", suffix)
        : join(app.getPath("userData"), "..", "Globalcord", suffix)
);
    ...
    if (!migrated) {
        setTimeout(() => {
            try {
                ...
                console.error("[Globalcord] Failed to copy prod data:", err);
            }
        }, 5000);
    }
}
