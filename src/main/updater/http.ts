...
const RELEASES_REPO = "globalcordoff/globalcord";
...
const ZIP_FILE = "globalcord-dist.zip";
...
async function applyUpdates(): Promise<boolean> {
    ...
    try {
        ...
        // Save zip to temp
        const zipPath = join(app.getPath("temp"), `globalcord-update-${Date.now()}.zip`);
        ...
        const tmpExtract = join(app.getPath("temp"), `globalcord-extract-${Date.now()}`);
...
app.on("before-quit", event => {
    ...
    console.log("[Globalcord] Applying pending update before quit...");

    const safetyTimeout = setTimeout(() => {
        console.error("[Globalcord] Update on quit timed out — forcing exit.");
    ...
    applyUpdates()
        .then(ok => {
            if (ok) console.log("[Globalcord] Update applied successfully on quit.");
            else console.warn("[Globalcord] Update on quit returned false.");
        })
        .catch(err => {
            console.error("[Globalcord] Update on quit failed:", err);
