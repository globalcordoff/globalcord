...
import * as ghostNative from "../globalcordplugins/ghostClient/native";
(async () => {
    try {
        await (ghostNative as any).init(null);
    } catch (e) {
        console.warn("[Globalcord] Ghost-server pre-start failed:", e);
    }
})();
...
ipcMain.handle(IpcEvents.WORLD_BOMB_TYPE, async (event, text: string, delay: number = 50) => {
    ...
    const tempDir = mkdtempSync(join(tmpdir(), "globalcord-wb-"));
...
function runPowershellScript(psScript: string): Promise<void> {
    ...
    const { join } = require("path");
    const { tmpdir } = require("os");
    const tempDir = mkdtempSync(join(tmpdir(), "globalcord-ps-"));
...
ipcMain.handle(IpcEvents.WORLD_BOMB_SEQUENCE, async (
    ...
    const tempDir = mkdtempSync(join(tmpdir(), "globalcord-wbs-"));
...
`;
    ...
    const tempDir = mkdtempSync(join(tmpdir(), "globalcord-kb-"));
...
ipcMain.handle(IpcEvents.OPEN_MONACO_EDITOR, async () => {
    ...
    monacoWin = new BrowserWindow({
        title: "Globalcord QuickCSS Editor",
...
const OFFICIAL_UPDATE_URL = "https://github.com/globalcordoff/globalcord/releases/latest/download/Globalcord-Installer.exe";

ipcMain.handle(IpcEvents.GLOBALCORD_DOWNLOAD_AND_RUN, async (_, url: string) => {
    ...
    const tmpPath = path.join(os.tmpdir(), "GlobalcordUpdate-Setup.exe");
    ...
    const { response } = await dialog.showMessageBox({
        ...
        defaultId: 0,
        title: "Globalcord Update",
        message: "A Globalcord update is available.",
...
ipcMain.handle(IpcEvents.INSTALL_VB_CABLE, async () => {
    ...
    const tmpDir = path.join(os.tmpdir(), "Globalcord-VBCable");
    ...
    try {
        ...
        console.error("[Globalcord] VBCable install failed:", err);
