...
function createGlobalcordMenuItems(): MenuItemConstructorOptions[] {
    return [
        {
            label: "Globalcord",
            submenu: [
                {
                    label: "About Globalcord",
                    click: () => openAboutWindow()
                },
                {
                    label: cachedUpdateAvailable ? "Update Globalcord" : "Check for Updates",
...
export function patchTrayMenu(): void {
    // No longer patching tray menu — Globalcord manages its own tray in patcher.ts
}
