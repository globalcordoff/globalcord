...
import { t, useTranslation } from "../autoTranslateGlobalcord";
...
function loadPersistLogs() {
    try {
        const s = localStorage.getItem("globalcord_logs");
        if (s) {
...
function savePersistLogs() {
    try {
        ...
        localStorage.setItem("globalcord_logs", JSON.stringify(toSave));
    } catch { }
}
...
function LogRow({ e }: { e: LogEntry; }) {
    ...
    const onDoubleClick = () => {
        ...
        try {
            ...
            console.error("Globalcord Navigation Error:", err);
...
function LogsModal({ rootProps }: { rootProps: any; }) {
    ...
    const saveAsTxt = useCallback(() => {
        try {
            ...
            a.href = url;
            a.download = `globalcord_logs_${new Date().toISOString().slice(0, 10)}.txt`;
            a.click();
...
export default definePlugin({
    ...
    start() {
        ...
        loadPersistLogs();
        addHeaderBarButton("globalcord-event-logs", () => <LogsButton />, 7);
        subscribeToEvents();
    },
    stop() {
        removeHeaderBarButton("globalcord-event-logs");
