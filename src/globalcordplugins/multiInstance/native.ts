...
import { registerMediaPermissionsForSession } from "../../globalcord/main/mediaPermissions";
...
function createTokenPreload(token: string): string {
    ...
    const dir = join(app.getPath("userData"), "globalcord-mi-preloads");
    mkdirSync(dir, { recursive: true });

    const safeToken = JSON.stringify(token); // échappe proprement le token

    const script = `
// Globalcord MultiInstance — token preload
// S'exécute dans le main world AVANT Discord
(function() {
    const TOKEN = ${safeToken};
    try {
        // Définit le token dans localStorage
        Object.defineProperty(window, '__globalcord_token', { value: TOKEN, writable: false });
        ...
        console.log("[GlobalcordMI] Token preload active ✓");
    } catch(e) {
        console.warn("[GlobalcordMI] Preload error:", e);
    }
})();
`;
...
export async function openInstanceWindow(
    ...
    try {
        ...
        const uniqueAppId = `globalcord.instance.${userId}.${Date.now()}`;
        ...
        const partition = `persist:globalcord-mi-${userId}`;
        ...
        const win = new BrowserWindow({
            ...
            title: `Globalcord [${username || userId}]`,
        ...
        if (process.platform === "win32") {
            try {
                win.setAppDetails({
                    ...
                    relaunchDisplayName: `Globalcord [${username || userId}]`,
                });
            } catch (err) {
                console.warn("[GlobalcordMI] setAppDetails failed:", err);
            }
        }
            ...
            wc.executeJavaScript(`
                (async () => {
                    ...
                    try {
                        ...
                        const ws = window.__GLOBALCORD_GW_WS__;
...
// ─────────────────────────────────────────────────────────────────────────────
// Fenetres « groupées » — meme groupe que Globalcord dans la barre des taches
// Principe : on ne touche PAS a setAppDetails => la fenetre herite de l'AppId
// du processus principal (com.globalcord.app), Windows la groupe automatiquement
// ─────────────────────────────────────────────────────────────────────────────
...
export async function arrangeSplit(_: any, userId: string): Promise<void> {
    try {
        ...
        console.error("[GlobalcordMI] arrangeSplit error:", e);
    }
}
