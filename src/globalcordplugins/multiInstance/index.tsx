...
const MI_TOKEN_CACHE_KEY = "globalcord-mi-token-cache";
...
function ContextMenuPortal(props: CtxMenuProps) {
    ...
    const [container] = React.useState(() => {
        const el = document.getElementById("globalcord-mi-ctx-root") ?? document.createElement("div");
        el.id = "globalcord-mi-ctx-root";
...
export default definePlugin({
    ...
    async start() {
        ...
        captureCurrentToken();
        addHeaderBarButton("globalcord-multi-instance", () => <MultiInstanceButton />, 9);
    },

    stop() {
        removeHeaderBarButton("globalcord-multi-instance");
        if (this._fluxUnsub) { this._fluxUnsub(); this._fluxUnsub = null; }
        const root = document.getElementById("globalcord-mi-ctx-root");
        if (root) root.remove();
    },
});
