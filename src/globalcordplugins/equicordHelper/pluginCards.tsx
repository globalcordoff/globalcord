...
import { EQUIBOT_USER_ID, GLOBALCORD_BOT_USER_ID } from "@utils/constants";
...
export function ChatPluginCard({ url, description }: { url: string, description: string; }) {
    ...
    if (excludedPlugin || !p) {
        const toolTipText = excludedPlugin
            ...
            : "This plugin is not on this version of Globalcord. Try updating!";
...
export const PluginCards = ErrorBoundary.wrap(function PluginCards({ message }: { message: Message; }) {
    ...
    // Process components — Globalcord Bot (globalcord.online, Component v2 Container format)
    if (message.author.id === GLOBALCORD_BOT_USER_ID) {
        ...
        if (containerComponents?.length >= 3) {
            ...
            if (pluginUrl?.startsWith("https://globalcord.online/plugins/")) {
