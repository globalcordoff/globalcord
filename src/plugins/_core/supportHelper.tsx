...
import { platformName } from "@globalcordplugins/equicordHelper/utils";
...
export default definePlugin({
    ...
    renderContributorDmWarningCard: ErrorBoundary.wrap(({ channel }) => {
        ...
        // Groupe 2 : Message "Globalcord developers"
        const globalcordGroupIds = [
        ...
        const isGlobalcordGroup = globalcordGroupIds.includes(userId);

        if (!isOriginalDev && !isGlobalcordGroup) return null;
        if (RelationshipStore.isFriend(userId) || isAnyPluginDev(UserStore.getCurrentUser()?.id)) return null;

        const developerText = isOriginalDev ? "Equicord & Vencord plugin developers" : "Globalcord developers";
