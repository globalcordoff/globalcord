...
try { _stealthActive = localStorage.getItem("Globalcord_stealthMode") === "1"; } catch { }
...
function persistStealth(v: boolean) {
    try { v ? localStorage.setItem("Globalcord_stealthMode", "1") : localStorage.removeItem("Globalcord_stealthMode"); } catch { }
}

// Éléments NON-React uniquement (titlebar, GlobalcordAI nav)
// NE PAS cacher les entrées settings sidebar — l'utilisateur doit pouvoir accéder aux paramètres
const NON_REACT_SELECTORS = [
    "#globalcord-titlebar-btn",
    "#globalcord-titlebar-link-style",
    ".nai-nav-item",
];
...
export function syncStealthBodyClass() {
    try { if (_stealthActive) document.body?.classList.add("globalcord-stealth"); else document.body?.classList.remove("globalcord-stealth"); } catch { }
...
if (_stealthActive) {
    ...
    try { document.body?.classList.add("globalcord-stealth"); } catch { }
}
...
try {
    ...
    window.addEventListener("globalcord-stealth-change", () => {
...
export function _notifyStealthChange() {
    ...
    window.dispatchEvent(new Event("globalcord-stealth-change"));
}
...
function HeaderBarButtons() {
    ...
    useEffect(() => {
        ...
        window.addEventListener("globalcord-stealth-change", listener);
        return () => {
            headerBarListeners.delete(listener);
            stealthListeners.delete(listener);
            window.removeEventListener("globalcord-stealth-change", listener);
        };
    }, []);
