...
const GLOBALCORD_RELEASES_REPO = "globalcordoff/globalcord";
const GLOBALCORD_REPO_URL = `https://github.com/${GLOBALCORD_RELEASES_REPO}`;
...
export { GLOBALCORD_REPO_URL };

export async function getCommitsSinceLastSeen(
    repoUrl: string,
): Promise<ChangelogEntry[]> {
    // Toujours utiliser le repo Globalcord, ignorer le repoUrl d'Equicord
    return fetchCommitsBetween(GLOBALCORD_RELEASES_REPO, "HEAD~10", "HEAD").catch(() => []);
}
