...
export const ExcludedReasons: Record<"web" | "discordDesktop" | "vesktop" | "equibop" | "desktop" | "dev", string> = {
    ...
    dev: "Developer version of Globalcord"
};
...
export default function PluginSettings({ premiumOnly = false }: PluginSettingsProps) {
    ...
    const [searchValue, setSearchValue] = useState({ value: "", status: SearchStatus.GLOBALCORD });
    ...
    React.useEffect(() => {
        ...
        window.addEventListener("globalcord-tutorial-detected", handler);
        return () => window.removeEventListener("globalcord-tutorial-detected", handler);
    }, []);
    ...
    const pluginFilter = useCallback((plugin: typeof Plugins[keyof typeof Plugins], newPluginsSet: Set<string> | null) => {
        ...
        switch (status) {
            ...
            case SearchStatus.ENABLED:
                if (!enabled) return false;
                break;
            case SearchStatus.GLOBALCORD:
                if (!PluginMeta[plugin.name].folderName.startsWith("src/globalcordplugins/")) return false;
                break;
            case SearchStatus.OTHERS:
                if (PluginMeta[plugin.name].folderName.startsWith("src/globalcordplugins/") || PluginMeta[plugin.name].folderName.startsWith("src/plugins/_")) return false;
    ...
    const { globalcordPlugins, othersPlugins, requiredPlugins } = useMemo(() => {
        const globalcordPlugins = [] as JSX.Element[];
        ...
        for (const p of sortedPlugins) {
            ...
            if (isRequired) {
                const tooltipText = p.required || !depMap[p.name]
                    ? "This plugin is required for Globalcord to function."
                ...
                const isGlobalcord = folderName.startsWith("src/globalcordplugins/");
                const card = (
                    <PluginCard
                        onRestartNeeded={handleRestartNeeded}
                        disabled={false}
                ...
                if (isGlobalcord) {
                    globalcordPlugins.push(card);
                } else {
                    othersPlugins.push(card);
                }
            }
        }
        return { globalcordPlugins, othersPlugins, requiredPlugins };
    ...
    const allPlugins = [...globalcordPlugins, ...othersPlugins];
    ...
    const globalcordVisible = globalcordPlugins.slice(0, Math.min(visibleCount, globalcordPlugins.length));
    const othersVisible = othersPlugins.slice(0, Math.max(0, visibleCount - globalcordPlugins.length));
        ...
        <SettingsTab>
                    ...
                    <ErrorBoundary noop>
                        <Select
                            options={[
                                ...
                                { label: "Show Globalcord Plugins", value: SearchStatus.GLOBALCORD },
            ...
            {premiumOnly ? (
                <>
                    ...
                    {globalcordPlugins.length || othersPlugins.length
                        ? (
                            <div className={cl("grid")}>
                                {[...globalcordVisible, ...othersVisible].length
                                    ? [...globalcordVisible, ...othersVisible]
                ...
                <>
                    {globalcordPlugins.length > 0 && (
                        <>
                            <HeadingTertiary className={Margins.top20}>Globalcord Plugins</HeadingTertiary>
                            <div className={cl("grid")}>
                                {globalcordVisible}
                            </div>
                        </>
                    ...
                    {globalcordPlugins.length === 0 && othersPlugins.length === 0 && (
                        <ExcludedPluginsList search={search} />
                    )}
